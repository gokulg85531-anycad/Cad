        ...
        public void Show(IRenderView render, MaterialInstance material)
        {
            ...
            else if(Type == "Text")
            {
                if(Content.Length > 0)
                {
                    ...
                    var trf = Matrix4d.makeTranslation(Data[0], Data[1], 0);
                    var height = 0.002 * Data[4];
                    var width = height / Data[5];
                    var scale = Matrix4d.makeScale(height, width, 1);
                    var rotate = Matrix4d.makeRotation(Vector3d.UNIT_X, new Vector3d(Data[2], Data[3], 0));
